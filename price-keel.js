...
(function() {
  var BLUE, BOLD, DARK_GREY, FONTHEIGHT, FONTWIDTH, FONT_FAMILY, GREEN, GREY, Graph, LIGHT_GREY, RED, REGULAR, YELLOW, abs, ceil, drawScreen, fit, fitClamp, floor, graph, log, log10, max, min, mouseDownAct, mouseUpAct, redrawScreen, resizeAct, round, timer, wheelAct;
  ...
  Graph = class Graph {
    ...
    draw() {
      var barHeight, barWidth, endAda, endPrice, endResultAda, endResultEur, endValue, f, fResult, fResultLeft, fResultOffset, fResultRight, fResultWidth, fTradeLeft, fTradeOffset, fTradeRight, fdt, fframe, fh, first, ftrade, ftradeWidth, ftv, fx, fy, fyPrice, fyRatio, fyRatioStr, gap, i, last, p, prec, range, s, scale, skip, startAda, startPrice, summary, t, x, y;
      ...
      while (i < this.duration) {
        ...
        y = this.priceToY(this.frames[this.start + i].mx);
        barHeight = this.priceToY(this.frames[this.start + i].mn) - y;
        if (this.start + i === f) {
          // Draw focus line
        ...
        if (this.frames[this.start + i].tv > 0) {
          this.ctx.fillStyle = GREEN;
        } else if (this.frames[this.start + i].tv < 0) {
          this.ctx.fillStyle = RED;
        } else {
          this.ctx.fillStyle = DARK_GREY;
        }
        this.ctx.fillRect(x + barWidth * gap / 2, y, barWidth * (1 - gap), barHeight);
        p = this.frames[this.start + i].wa;
        this.ctx.fillStyle = LIGHT_GREY;
        this.ctx.fillRect(x + barWidth * gap / 2, this.priceToY(p), barWidth * (1 - gap), 1);
        i++;
      }
      ...
      if (this.yCoord > this.padTop - 10 && this.yCoord < this.canvas.height - this.padBottom + 10) {
        this.ctx.lineWidth = .5;
        fyPrice = fit(this.yCoord, this.canvas.height - this.padBottom, this.padTop) * (this.maximum - this.minimum) + this.minimum;
        fyRatio = fitClamp(log(fyPrice), log(this.bottom), log(this.top));
        this.drawHorizLine(fyPrice, DARK_GREY);
        if (fyRatio >= 1) {
          fyRatioStr = "1.00";
        } else {
          fyRatioStr = fyRatio.toFixed(3).slice(1);
        }
        this.ctx.fillStyle = DARK_GREY;
        this.ctx.fillText(fyRatioStr, this.canvas.width - this.padRight / 2, this.yCoord);
      }
